...
struct GameObject
{
    ...
    int state;  // 0: Bơi (Swimming), 1: Nhảy (Jumping)
};
...
void drawSea()
{
    ...
    int stacks = 20;
    float radiusBase = 600.0f;
    float length = 1200.0f; // Biển dài hơn
    float zStart = -600.0f;
...
void drawFish(const GameObject& f) {
    ...
    if (f.state == 0) { // Đang bơi
        // Cá nổi lềnh bềnh, lắc nhẹ
        glRotatef(sin(wavePhase * 2.0f) * 10.0f, 0.0f, 0.0f, 1.0f);
    } else { // Đang nhảy
        // Xoay đầu theo hướng nhảy
        float angle = atan2(f.vy, 5.0f) * 180.0f / M_PI; 
        glRotatef(angle, 0.0f, 0.0f, 1.0f);
    }

    glColor3f(0.2f, 0.4f, 0.8f); // Màu xanh dương đậm
    ...
    // Mắt
    glColor3f(1.0f, 1.0f, 1.0f);
    glPushMatrix(); glTranslatef(8.0f, 3.0f, 3.0f); glutSolidSphere(2.0, 5, 5); glPopMatrix();
    glPushMatrix(); glTranslatef(8.0f, 3.0f, -3.0f); glutSolidSphere(2.0, 5, 5); glPopMatrix();
...
void update(int value)
{
    ...
    fuel -= (0.05f * gameSpeed);
    if (fuel <= 0.0f) { fuel = 0.0f; isGameOver = true; }
    ...
    // --- SINH CÁ ---
    fishSpawnTimer++;
    if (fishSpawnTimer > (120.0f / gameSpeed)) { // Cá xuất hiện thường xuyên
        GameObject f;
        f.x = 400.0f; // Từ xa bên phải
        f.y = -5.0f;  // Nổi trên mặt nước
        f.z = (rand() % 100) - 50.0f; 
        f.active = true;
        f.scale = 1.0f;
        f.vy = 0.0f;
        f.state = 0; // Trạng thái bơi
        fishes.push_back(f);
        fishSpawnTimer = 0;
    }
    ...
    // --- LOGIC CÁ: BƠI & NHẢY ---
    for (auto &f : fishes) {
        f.x -= (3.0f * gameSpeed); // Trôi theo biển
        
        if (f.state == 0) { // Đang bơi
            // Phát hiện máy bay (khi cá đến gần máy bay theo trục X)
            // Máy bay cố định X = -50. Khi cá đến khoảng X = 80, nó sẽ nhảy
            if (f.x < 80.0f && f.x > -100.0f) { 
                f.state = 1; // Chuyển sang nhảy
                f.vy = 12.0f + (rand() % 8); // Lực nhảy ngẫu nhiên
            }
        } else { // Đang nhảy
            f.y += f.vy;
            f.vy -= 0.5f; // Trọng lực kéo xuống
            
            // Nếu rơi xuống sâu thì mất
            if (f.y < -100.0f) f.active = false;
        }
    }
